...
_LEGACY_ROOT = Path.home() / "Documents" / "MultiVerse"
...
def _migrate_legacy_user_data() -> None:
    ...
    if _USER_ROOT.exists() and any(_USER_ROOT.iterdir()):
        return
    try:
        shutil.copytree(_LEGACY_ROOT, _USER_ROOT, dirs_exist_ok=True)
        logger.info("Migrated user data from %s to %s", _LEGACY_ROOT, _USER_ROOT)
    except Exception:
...
def user_data_root() -> Path:
    ...
    if not _USER_ROOT.exists() and _LEGACY_ROOT.exists():
        try:
            _USER_ROOT.mkdir(parents=True, exist_ok=True)
            for item in _LEGACY_ROOT.iterdir():
